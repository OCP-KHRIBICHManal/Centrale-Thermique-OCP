...
with tab4:
    ...
    if not pymoo_ok:
        ...
        if "pareto_F" in st.session_state:
            ...
            import io as _io
            rows_dl = [{"Solution": "Point actuel",
                        "x1 Turbine (T/h)": sa["x1"], "x2 Bypass MP (T/h)": sa["x2"],
                        "x3 Bypass BP (T/h)": sa["x3"], "x4 Sout. MP (T/h)": sa["x4"],
                        "x5 Sout. BP (T/h)": sa["x5"], "x6 Condenseur (T/h)": sa["x6"],
                        "P_elec (MW)": sa["P"], "Déficit (T/h)": sa["D"]}]
            for i in range(len(F)):
                x1r,x4r,x5r,x2r,x3r = Xr[i]
                lbl = f"S{i+1}" + (" ★ Max P" if i==idx_maxP else " ◆ Comp." if i==idx_knee else " ▼ Min D" if i==idx_minD else "")
                rows_dl.append({"Solution": lbl,
                                 "x1 Turbine (T/h)": round(x1r,1), "x2 Bypass MP (T/h)": round(x2r,1),
                                 "x3 Bypass BP (T/h)": round(x3r,1), "x4 Sout. MP (T/h)": round(x4r,1),
                                 "x5 Sout. BP (T/h)": round(x5r,1), "x6 Condenseur (T/h)": round(max(x1r-x4r-x5r,0),1),
                                 "P_elec (MW)": round(P_vals[i],2), "Déficit (T/h)": round(def_vals[i],1)})
            df_dl = pd.DataFrame(rows_dl)
            csv_buf = _io.StringIO()
            df_dl.to_csv(csv_buf, index=False, sep=";", decimal=",")
            st.download_button(
                label="⬇️ Télécharger les solutions (CSV)",
