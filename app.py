      ...
      <input type="text" id="ems-search" placeholder="&#128269; Onglet..." autocomplete="off"
        ...
        oninput="(function(){
          ...
          function tryClick(){
            ...
            if(best){best.click();inp.value='';return true;}
            return false;
          }
          if(!tryClick()){
            var n=0;
            var id=setInterval(function(){
              if(tryClick()||++n>10)clearInterval(id);
            },150);
          }
        })()" />
    </div>
        ...
        with col_r:
            if run_p:
                class ParetoTurbine(Problem):
                    def __init__(self):
                        super().__init__(
                            n_var=5, n_obj=2, n_ieq_constr=5,
                            xl=np.array([100., 0., 0., 0., 0.]),
                            xu=np.array([Q_HP_p, 12., 57., 80., 120.])
                        )
                    def _evaluate(self, X, out, *args, **kwargs):
                        x1r, x4r, x5r, x2r, x3r = X[:,0], X[:,1], X[:,2], X[:,3], X[:,4]
                        P_mw = np.maximum(((x1r-x4r-x5r)/3.5)+(x4r/8)+(x5r/6.5), 0)
                        def_MP = np.maximum(D_MP - (x2r + x4r + Q_JPH_recep), 0)
...
<div style="text-align:center;color:#888;font-size:0.8rem;padding:0.5rem">
    EMS — Centrale Thermique OCP | Turbine GE Nuovo Pignone 3000 RPM | UNITE 515A<br>
    Développé dans le cadre du PFE — Master Génie Énergétique
</div>
""", unsafe_allow_html=True)
